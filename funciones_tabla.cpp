#include <iostream>
#include "funciones_primarias.cpp"
#include <cstdlib>
using namespace std;
...
void expandir_memoria(unsigned char ** ptr_contenido, short int * bytes_reservados,short int * ptr_num_filas,
     short int * ptr_num_columnas,short int bytes_necesarios, short int * ptr_num_expo){

    if (bytes_necesarios > *bytes_reservados){// esto enrealidad toca ponerlo en agregar fila/columna y despues si si invocar la funcion
        if ((bytes_necesarios+5)/6 > *ptr_num_expo){ //miramos si cabe entre los bloques que ya tenemos
            ...
            for(short int = 0; i < *ptr_num_expo ){
                contenido[i] = ptr_contenido[i];
            }
            delete[] ptr_contenido;
            ptr_contenido = new unsigned char[*ptr_num_expo * 2];
            
            for(short int i = 0; i < *ptr_num_expo ){
                ptr_contenido[i] = contenido[i];
            }
            for(short int i = ((*bytes_reservados+5)/6); i < (((bytes_necesarios-*bytes_reservados)+5)/6); i++){ 
            ...
            *ptr_num_expo *= 2;
            ...
            for(short int i = ((*bytes_reservados+5)/6); i < (((bytes_necesarios-*bytes_reservados)+5)/6); i++){//desde el ultimo bloque ya existente hasta todos los bloques necesarios 
                ptr_contenido[i] = new unsigned char[6];
            }
        }
...
unsigned char ** agregar_fila(unsigned char ** ptr_contenido, short int fila_remplazar,short int * ptr_num_filas, short int * ptr_num_columnas,
    ...
    *ptr_num_filas += 1;
    ...
    expandir_memoria(ptr_contenido,bytes_reservados,ptr_num_filas,ptr_num_columnas,bytes_necesarios,ptr_num_expo);
    }
...
unsigned char ** quitar_fila(unsigned char ** ptr_contenido, short int fila_quitar,short int * ptr_num_filas,
    ...
    *ptr_num_filas -= 1;
    for(short int fila = fila_quitar + 1; fila < *ptr_num_filas,fila++){
        for(short int columna = *ptr_num_columnas; columna > 0; columna--){
